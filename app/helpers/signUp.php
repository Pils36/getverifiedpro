...
function signUp()
{
	...
	if ($_POST['email'] == "") {
		...
		$myResponse->message = "Please enter a valid email address";
	...
	if ($_POST['password'] == "") {
		$myResponse->status = "failed";
		$myResponse->message = "Please use a password";
	...
	if ($row_count > 0) {
		$myResponse->status = "failed";
		$myResponse->message = "This email has already been used";
		return json_encode($myResponse);
		...
		try {
			...
			$stmt->execute(array(
				...
				":email" => $_POST['email'],
				":link" => $link,
				":type" => !empty($_POST['account_type']) ? $_POST['account_type'] : NULL
			));
...
//			$response = json_encode('success');
			...
			$myResponse->message = $response;
			$myResponse->data = array('login_id' => $insertId);
			return json_encode($myResponse);
...
function verifyEmail()
{
	...
	$link = $_POST['link'];
	$link = "http://pro-filr.com/verifyemail?link=" . $link;
	...
	try {
		...
		if ($affected_rows < 1) {
			$myResponse->status = "success";
			$myResponse->message = "This link is no longer valid. Please <a class='nav_link' data-page='index' data-view='signup'>signup</a> again to receive a fresh email verification link";
			return json_encode($myResponse);
		}
		...
		$dbh->commit();
		$myResponse->status = "success";
		$myResponse->message = "<h3>Your email was successfully verified.</h3> Please <a class='nav_link' data-page='index' data-view='login' href='login.php'>login</a> to complete your profile";
		//send welcome email
		sendWelcome($link);
...
function verifyLinkGenerator($email)
{
	...
	return "http://pro-filr.com/verifyemail?link=$link";
	
}


function sendWelcome($link)
{
	$dbh = Database::getInstance();
	$myResponse = new Response();
	//return $link;
	...
	$email = $row['email'];
	$body = file_get_contents(DOCUMENT_ROOT . 'templates/Welcome/Welcome.html');
	$mail = new PHPMailer;
	$mail->isSMTP();
