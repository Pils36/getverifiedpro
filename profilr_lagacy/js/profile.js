...
(function () {
    this.onReady = function () {
        ...
        sendRequest.postJSON(profileData, "../helpers/loader.php", function (response) {
            var parse = response;
            ...
            myProfile.affliationTemplate(parse);
            myProfile.projectTemplate(parse);
            if (parse.data.subscription.active == "0") {
                $(".profile_menu").prepend("<div class='item'> <a href='upgrade.php' class='ui button upgrade' style='color:white; background-color: #ff4e00 !important'>UPGRADE ACCOUNT</a> </div>");
            }
            NProgress.done();
            $(".Site-content").show();
            myProfile.readyList();
        ...
        $("#save_btn").on("click", function () {
            ...
            sendRequest.postJSON(data, "../helpers/loader.php", function (response) {
                ...
                switch (table) {
                    ...
                    case "tbl_affliation":
                        //code block
                        myProfile.affliationTemplate(rows, "single");
                        break;
        ...
        $(".profile_link").on("click", function () {
            view = $(this).data("view") + ".php";
            $.redirect(view, {"fromNav": "1"});
            // $("spcontent").load("views/"+view+".html");
            //alert("me");
        ...
        this.affliationTemplate = function (parse, dataType="all") {
            if (dataType == "single") {
                data = parse;
            } else {
            ...
            $.each(data, function (v, k) {
                ...
                strItem += "<div class='ui list user-info twelve wide column'><div class='item'><div class='ui horizontal bulleted list'><div class='item' style='font-weight:bold;'>" + k.organisation + "</div>" + strGroup + "<div class='item'>Member Since: " + k.year_joined + "</div></div></div>" + strDesc + "</div><div class='ui two wide column'> <button class='circular ui icon mini right floated basic button edit_btn' data-content='affliation' data-ref='" + k.id + "'> <i class='icon write'></i> </button> </div><div class='ui two wide column'> <button class='circular ui icon mini left floated basic button delete_btn' data-content='affliation' data-ref='" + k.id + "'>  <i class='icon remove'></i> </button> </div>";
            })
            strList += strItem;
            $("#affliation_row").html(strList);
                ...
                sendRequest.postJSON(data, "../helpers/loader.php", function (response) {
                    ...
                    switch (response.data.table) {
                        case "education":
                            ...
                            break;
                        case "affliation":
                            header = "Religious/Social/Charitable Affliation";
                            page = "affliation";
                            break;
                        case "experience":
                            header = "Industry Experience (Previous)";
                            page = "industry";
                            break;
                        case "certification":
        ...
        this.industryExperience = function (el) {
            $("#experience_modal > .content").load("../modals/industry.html", function () {
                $('.coupled.modal').modal({
                    allowMultiple: true
                });
    ...
    this.industryProject = function () {
        ...
        sendRequest.postJSON(data, "../helpers/loader.php", function (response) {
            ...
            $("#modal_description").load("../modals/project.html", function () {
        ...
        this.loadModal = function (el, strInd="", strYr="", strEmployer="") {
            ...
            //var that = $(this);
            var content = "../modals/" + $(el).data("content") + ".html";
            $("#modal_description").load(content, function () {
                $("#profile_modal").modal({
                    closable: false,
...
$(document).ready(function () {
    ...
    myProfile.onReady();
    //$("[data-view='profile2']").trigger("click");
