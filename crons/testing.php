...
try {
	...
	$rows = $dbh->query("SELECT upper(lastname) AS lastname,upper(firstname) AS firstname,email FROM tbl_account_individual WHERE (profession IS NULL OR country IS NULL OR industry IS NULL OR company IS NULL or phone is NULL OR photo = 'profile-placeholder.png' OR position is NULL OR city IS NULL) LIMIT 2")->fetchAll(PDO::FETCH_ASSOC);
	...
	$body = file_get_contents('../templates/Complete/testing.html');
...
function cron_($data, $subject, $body)
{
	...
	$mail->setFrom('subscription@profilr.com', 'Profilr Inc.');
// 	$mail->msgHTML($body);
    ...
    // print_r($member);
        // $email = "profilr2019@gmail.com";
        $email = "ins-xagndr3i@isnotspam.com";
        
        // $email = $member['email'];
		$body = str_replace(
            array('_firstname_', '_lastname_', '_email_'),
            array($member['firstname'],    $member['lastname'], $member['email']),
            file_get_contents('../templates/Complete/testing.html')
        );
