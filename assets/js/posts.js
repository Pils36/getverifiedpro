...
post.posts = function (type, pageno = 1) {
    ...
    sendRequest.postJSON(data, "controller/app.php", function (response) {
        ...
        if (response.status === "success") {
            ...
            $("#post_segment").html("<h3 style='color:red'>"+response.message+"</h3>");
        }else {
            //fetch failed
...
post.mineEdit = function (el) {
    if ($(el).data("pub") === "published") {
        swal("Oops","Published posts cannot be edited", "info");
        return;
    }
    var data = {
        "function": "editRecord",
...
$("#save_btn").click(function(){
    var data = {
        ...
        "thissubject": $("#subject").val(),
        "thisdescription": $("#description").val(),
        "thislocation": $("#location").val(),
        "thiszip": $("#zip").val(),
        "thisdeadline": $("#deadline").val(),
        "thisindustry": $("#industry").val(),
        "thisspecialisation": $("#specialisation").val(),
        "thisrequirement": $("#requirement").val(),
    }
    sendRequest.postJSON(data, "controller/app.php", function (response){
        console.log(data);
        if(response.status == "success"){
            ...
            $("#interest_modal").modal({
                ...
                onVisible: function (elem) {
                    ...
                    $.each(response.data, function (v, k) {
                        photo = "assets/resources/pics/" + k.photo;
...
post.viewMember = function (el) {
    ...
    $.redirect("member", {id: $(el).data("id")}, "post");
};
...
post.messages = function (type) {
    ...
    sendRequest.postJSON(data, "controller/app.php", function (response) {
        if(response.status==="inactive"){
            $("#message_segment").html("<h3 style='color:red'>"+response.message+"</h3>");
            return;
        }
        if (response.status === "failed") {
            alert(response.message);
